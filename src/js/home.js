...
async function getAllProducts() {
  ...
  for (const cat of categories) {
    ...
    const items = (Array.isArray(data) ? data : data.Result || []).map(
      (item) => ({ ...item, _category: cat }),
    );
...
function renderTopProducts(products) {
  const top = products
    .filter(
      (p) =>
        (p.FinalPrice || p.ListPrice || p.SuggestedRetailPrice) &&
        p.Id &&
        typeof p.Id === "string" &&
        p.Id.trim() !== "",
    )
    .sort(
      (a, b) =>
        (a.FinalPrice || a.ListPrice || a.SuggestedRetailPrice) -
        (b.FinalPrice || b.ListPrice || b.SuggestedRetailPrice),
    )
    .slice(0, 4);
  ...
  top.forEach((product) => {
    const image =
      (product.Images && product.Images.PrimaryLarge) || product.Image || "";
